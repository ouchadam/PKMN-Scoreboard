package com.ouchadam.psr.read;

import java.util.ArrayList;
import java.util.List;

public class Pokedex {

    static List<String> POKEDEX = new ArrayList<String>(151);
    private static final int ZERO_INDEX_COEFF = 1;

    static {
        POKEDEX.add("Bulbasaur");
        POKEDEX.add("Ivysaur");
        POKEDEX.add("Venusaur");
        POKEDEX.add("Charmander");
        POKEDEX.add("Charmeleon");
        POKEDEX.add("Charizard");
        POKEDEX.add("Squirtle");
        POKEDEX.add("Wartortle");
        POKEDEX.add("Blastoise");
        POKEDEX.add("Caterpie");
        POKEDEX.add("Metapod");
        POKEDEX.add("Butterfree");
        POKEDEX.add("Weedle");
        POKEDEX.add("Kakuna");
        POKEDEX.add("Beedrill");
        POKEDEX.add("Pidgey");
        POKEDEX.add("Pidgeotto");
        POKEDEX.add("Pidgeot");
        POKEDEX.add("Rattata");
        POKEDEX.add("Raticate");
        POKEDEX.add("Spearow");
        POKEDEX.add("Fearow");
        POKEDEX.add("Ekans");
        POKEDEX.add("Arbok");
        POKEDEX.add("Pikachu");
        POKEDEX.add("Raichu");
        POKEDEX.add("Sandshrew");
        POKEDEX.add("Sandslash");
        POKEDEX.add("Nidoran");
        POKEDEX.add("Nidorina");
        POKEDEX.add("Nidoqueen");
        POKEDEX.add("Nidoran");
        POKEDEX.add("Nidorino");
        POKEDEX.add("Nidoking");
        POKEDEX.add("Clefairy");
        POKEDEX.add("Clefable");
        POKEDEX.add("Vulpix");
        POKEDEX.add("Ninetales");
        POKEDEX.add("Jigglypuff");
        POKEDEX.add("Wigglytuff");
        POKEDEX.add("Zubat");
        POKEDEX.add("Golbat");
        POKEDEX.add("Oddish");
        POKEDEX.add("Gloom");
        POKEDEX.add("Vileplume");
        POKEDEX.add("Paras");
        POKEDEX.add("Parasect");
        POKEDEX.add("Venonat");
        POKEDEX.add("Venomoth");
        POKEDEX.add("Diglett");
        POKEDEX.add("Dugtrio");
        POKEDEX.add("Meowth");
        POKEDEX.add("Persian");
        POKEDEX.add("Psyduck");
        POKEDEX.add("Golduck");
        POKEDEX.add("Mankey");
        POKEDEX.add("Primeape");
        POKEDEX.add("Growlithe");
        POKEDEX.add("Arcanine");
        POKEDEX.add("Poliwag");
        POKEDEX.add("Poliwhirl");
        POKEDEX.add("Poliwrath");
        POKEDEX.add("Abra");
        POKEDEX.add("Kadabra");
        POKEDEX.add("Alakazam");
        POKEDEX.add("Machop");
        POKEDEX.add("Machoke");
        POKEDEX.add("Machamp");
        POKEDEX.add("Bellsprout");
        POKEDEX.add("Weepinbell");
        POKEDEX.add("Victreebel");
        POKEDEX.add("Tentacool");
        POKEDEX.add("Tentacruel");
        POKEDEX.add("Geodude");
        POKEDEX.add("Graveler");
        POKEDEX.add("Golem");
        POKEDEX.add("Ponyta");
        POKEDEX.add("Rapidash");
        POKEDEX.add("Slowpoke");
        POKEDEX.add("Slowbro");
        POKEDEX.add("Magnemite");
        POKEDEX.add("Magneton");
        POKEDEX.add("Farfetch");
        POKEDEX.add("Doduo");
        POKEDEX.add("Dodrio");
        POKEDEX.add("Seel");
        POKEDEX.add("Dewgong");
        POKEDEX.add("Grimer");
        POKEDEX.add("Muk");
        POKEDEX.add("Shellder");
        POKEDEX.add("Cloyster");
        POKEDEX.add("Gastly");
        POKEDEX.add("Haunter");
        POKEDEX.add("Gengar");
        POKEDEX.add("Onix");
        POKEDEX.add("Drowzee");
        POKEDEX.add("Hypno");
        POKEDEX.add("Krabby");
        POKEDEX.add("Kingler");
        POKEDEX.add("Voltorb");
        POKEDEX.add("Electrode");
        POKEDEX.add("Exeggcute");
        POKEDEX.add("Exeggutor");
        POKEDEX.add("Cubone");
        POKEDEX.add("Marowak");
        POKEDEX.add("Hitmonlee");
        POKEDEX.add("Hitmonchan");
        POKEDEX.add("Lickitung");
        POKEDEX.add("Koffing");
        POKEDEX.add("Weezing");
        POKEDEX.add("Rhyhorn");
        POKEDEX.add("Rhydon");
        POKEDEX.add("Chansey");
        POKEDEX.add("Tangela");
        POKEDEX.add("Kangaskahn");
        POKEDEX.add("Horsea");
        POKEDEX.add("Seadra");
        POKEDEX.add("Goldeen");
        POKEDEX.add("Seaking");
        POKEDEX.add("Staryu");
        POKEDEX.add("Starmie");
        POKEDEX.add("Mr.Mime");
        POKEDEX.add("Scyther");
        POKEDEX.add("Jynx");
        POKEDEX.add("Electabuzz");
        POKEDEX.add("Magmar");
        POKEDEX.add("Pinsir");
        POKEDEX.add("Tauros");
        POKEDEX.add("Magikarp");
        POKEDEX.add("Gyarados");
        POKEDEX.add("Lapras");
        POKEDEX.add("Ditto");
        POKEDEX.add("Eevee");
        POKEDEX.add("Vaporeon");
        POKEDEX.add("Jolteon");
        POKEDEX.add("Flareon");
        POKEDEX.add("Porygon");
        POKEDEX.add("Omanyte");
        POKEDEX.add("Omastar");
        POKEDEX.add("Kabuto");
        POKEDEX.add("Kabutops");
        POKEDEX.add("Aerodactyl");
        POKEDEX.add("Snorlax");
        POKEDEX.add("Articuno");
        POKEDEX.add("Zapdos");
        POKEDEX.add("Moltres");
        POKEDEX.add("Dratini");
        POKEDEX.add("Dragonair");
        POKEDEX.add("Dragonite");
        POKEDEX.add("Mewtwo");
        POKEDEX.add("Mew");

        toLowercase(POKEDEX);
    }

    private static void toLowercase(List<String> pokemon) {
        for (int index = 0; index < pokemon.size(); index++) {
            String temp = pokemon.get(index).toLowerCase();
            pokemon.set(index, temp);
        }
    }

    public int getIdFor(String pokemonName) {
        return POKEDEX.indexOf(pokemonName.toLowerCase()) + ZERO_INDEX_COEFF;
    }

    public boolean has(String pokemon) {
        return POKEDEX.contains(pokemon.toLowerCase());
    }

}
