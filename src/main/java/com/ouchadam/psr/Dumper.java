...
import com.ouchadam.psr.read.file.RandomFileAccessWrapper;
...
public class Dumper {
    ...
    public static void dumpAll(long start, long end, RandomFileAccessWrapper file) throws IOException {
    ...
    public static void printAllText(long start, long end, RandomFileAccessWrapper file) {
        String text = "";
...
//                    System.out.println(saveByte);
    ...
    public static void dumpMoney(PokemonState state, Money wanted) {
        System.out.println("Start");
        for (int i = 0; i < state.length() - 3; i++) {
            MoneyReader moneyReader = new MoneyReader(i);
            try {
                Money money = moneyReader.read(state);
    ...
    public static void dumpPlaytime(PokemonState state, Playtime wanted) {
        System.out.println("Start");
        for (int i = 0; i < state.length() - 3; i++) {
            PlaytimeReader moneyStateReader = new PlaytimeReader(i);
            try {
                Playtime playtime = moneyStateReader.read(state);
    ...
    public static void dumpPokemon(PokemonState state, String name, int level) {
        System.out.println("Start");
        TextReader textReader = new TextReader();
        ...
        for (int i = 0; i < state.length() - 3; i++) {
            ...
            try {
                Team team = moneyStateReader.read(state);
